...
const AUTH_APPLICATION_DIR = path.resolve(__dirname, "../../src/modules/auth/application");
...
describe("auth application boundary", () => {
  it("does not import infra layer directly", () => {
    const applicationFiles = collectTsFiles(AUTH_APPLICATION_DIR);
    ...
    for (const applicationFile of applicationFiles) {
      ...
      if (source.includes("/infra/") || source.includes("../infra/")) {
        violations.push(path.relative(AUTH_APPLICATION_DIR, applicationFile));
      }
    }
