...
import { deleteEntityImageUseCase, uploadEntityImageUseCase } from "../../../src/usecases/entity-images-usecase";
...
describe("entity-images usecase cleanup behavior", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("schedules cleanup and returns success when image deletion on R2 fails", async () => {
    ...
    const result = await deleteEntityImageUseCase(db, imageBucket, "entity-1", "img-1");
  ...
  it("schedules cleanup when metadata insert fails and rollback delete fails", async () => {
    ...
    const result = await uploadEntityImageUseCase(db, imageBucket, "entity-1", file);
