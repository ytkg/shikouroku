...
import {
  createD1EntityApplicationRepository,
  createD1EntityReadRepository
...
import { createD1EntityImageRepository } from "../../modules/catalog/image/infra/image-repository-d1";
...
function createEntityWriteDependencies(db: D1Database) {
  return {
    entityRepository: createD1EntityApplicationRepository(db),
...
function createImageDependencies(db: D1Database) {
  return {
    entityReadRepository: createD1EntityReadRepository(db),
    entityImageRepository: createD1EntityImageRepository(db),
...
export function createEntityRoutes(): Hono<AppEnv> {
  ...
  entities.get("/entities", async (c) => {
    ...
    const entityRepository = createD1EntityApplicationRepository(c.env.DB);
    const result = await listEntitiesQuery(entityRepository, resolvedQuery.data);
    if (!result.ok) {
  ...
  entities.get("/entities/:id", async (c) => {
    ...
    const entityRepository = createD1EntityApplicationRepository(c.env.DB);
    const result = await getEntityQuery(entityRepository, id);
    if (!result.ok) {
      return useCaseError(c, result.status, result.message);
    }
  ...
  entities.post("/entities", async (c) => {
    ...
    const { entityRepository, kindRepository, tagRepository } = createEntityWriteDependencies(c.env.DB);
    const result = await createEntityCommand(entityRepository, kindRepository, tagRepository, parsedBody.data);
    if (!result.ok) {
      return useCaseError(c, result.status, result.message);
    }
  ...
  entities.patch("/entities/:id", async (c) => {
    ...
    const { entityRepository, kindRepository, tagRepository } = createEntityWriteDependencies(c.env.DB);
    const result = await updateEntityCommand(entityRepository, kindRepository, tagRepository, id, parsedBody.data);
    if (!result.ok) {
      return useCaseError(c, result.status, result.message);
    }
  ...
  entities.get("/entities/:id/images", async (c) => {
    const id = c.req.param("id");
    const { entityReadRepository, entityImageRepository } = createImageDependencies(c.env.DB);
    const result = await listEntityImagesQuery(entityReadRepository, entityImageRepository, id);
    if (!result.ok) {
      return useCaseError(c, result.status, result.message);
    }
  ...
  entities.patch("/entities/:id/images/order", async (c) => {
    ...
    const result = await reorderEntityImagesCommand(
      ...
      entityImageRepository,
      id,
      parsedBody.data.orderedImageIds
    );
    if (!result.ok) {
      return useCaseError(c, result.status, result.message);
    }
